...
then
    ...
    export SEDCMD="s#bootopt=64S3,32N2,64N2 androidboot.selinux=enforcing#$CMDLINE#g"
    echo $SEDCMD > $TMPDIR/sedcommand
    cp $TMPDIR/$BOOTIMAGEFILE-Image.gz $DEVICE_DIR/kernel
    popd > /dev/null
else
    mkdir -p $DEVICE_DIR
...
do
    ...
    cat $file | sed s/jadelte/$DEVICE/g | sed s/samsung/$MANUFACTURER/g | sed -f $TMPDIR/sedcommand | sed s/0x40078000/$BASE/g | sed s/2048/$PAGESIZE/g > $OUTPUT_FILE
done
...
echo '  'lunch lineage_$DEVICE-eng
